...
function isNearResistance(currentPrice, resistances) {
    for (const level of resistances) {
        if (isInZone(currentPrice, level.zoneLow, level.zoneHigh) || currentPrice < level.zoneLow) {
            return true;
        }
    }
    return false;
...
function isNearSupport(currentPrice, supports) {
    for (const level of supports) {
        if (isInZone(currentPrice, level.zoneLow, level.zoneHigh) || currentPrice > level.zoneHigh) {
            return true;
        }
    }
    return false;
