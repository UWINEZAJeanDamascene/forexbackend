...
const BAND_THRESHOLDS = { low: 40, moderate: 70 };
...
function computeTrendAlignment(trend) {
    const factors = [
...
function computeMarketStructure(structure, warnings) {
    ...
    if (structure.events.length > 0) {
        ...
        const contradicts = (structure.trend === 'bullish' && (latestEvent === 'lower_high' || latestEvent === 'lower_low')) ||
        ...
        else if ((structure.trend === 'bullish' && (latestEvent === 'higher_high' || latestEvent === 'higher_low')) ||
...
function computeSupportResistance(sr, currentPrice, warnings) {
...
function computeMomentumAgreement(momentum, trend) {
...
function computeVolatility(volatility) {
...
function computeMultiTimeframe(mtf, warnings) {
    const snapshots = [];
...
function checkOpposingSetups(setups) {
...
//# sourceMappingURL=confidenceAnalysisEngine.js.map