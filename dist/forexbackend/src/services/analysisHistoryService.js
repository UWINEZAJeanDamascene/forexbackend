...
exports.getAnalysisDetail = getAnalysisDetail;
exports.deleteAnalysis = deleteAnalysis;
const prisma_1 = require("../database/prisma");
const analysisEngineVersion_1 = require("@shared/constants/analysisEngineVersion");
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)('analysisHistory');
function buildAnalysisData(request) {
    const data = {
        ...
        engineVersion: analysisEngineVersion_1.ANALYSIS_ENGINE_VERSION,
    };
    if (request.marketDataTimestamp)
        data.marketDataTimestamp = new Date(request.marketDataTimestamp);
    if (request.dataFreshnessMs !== undefined)
        data.dataFreshnessMs = request.dataFreshnessMs;
    if (request.trendStrength)
        data.trendStrength = request.trendStrength;
    if (request.trendScore !== undefined)
        data.trendScore = request.trendScore;
    if (request.trendFactors)
        data.trendFactors = request.trendFactors;
    if (request.momentumScore !== undefined)
        data.momentumScore = request.momentumScore;
    if (request.momentumStrength)
        data.momentumStrength = request.momentumStrength;
    if (request.volatilityClassification)
        data.volatilityClassification = request.volatilityClassification;
    if (request.volatilityScore !== undefined)
        data.volatilityScore = request.volatilityScore;
    if (request.structureTrendQualifier)
        data.structureTrendQualifier = request.structureTrendQualifier;
    if (request.confidenceBand)
        data.confidenceBand = request.confidenceBand;
    if (request.tradeQualityVerdict)
        data.tradeQualityVerdict = request.tradeQualityVerdict;
    if (request.tradeQualityReasons)
        data.tradeQualityReasons = request.tradeQualityReasons;
    if (request.setupName)
        data.setupName = request.setupName;
    if (request.setupDirection)
        data.setupDirection = request.setupDirection;
    if (request.setupStrength !== undefined)
        data.setupStrength = request.setupStrength;
    if (request.setupConditionsMet)
        data.setupConditionsMet = request.setupConditionsMet;
    if (request.setupConditionsMissing)
        data.setupConditionsMissing = request.setupConditionsMissing;
    if (request.setupInvalidation)
        data.setupInvalidation = request.setupInvalidation;
    if (request.setups)
        data.setups = request.setups;
    if (request.aiExplanation)
        data.aiExplanation = request.aiExplanation;
    if (request.aiProvider)
        data.aiProvider = request.aiProvider;
    if (request.aiStructured)
        data.aiStructured = request.aiStructured;
    if (request.aiDisclaimer)
        data.aiDisclaimer = request.aiDisclaimer;
    return data;
}
async function saveAnalysis(request, userId) {
    try {
        const snapshotKey = request.marketDataTimestamp ? `${request.symbol}:${request.timeframe}:${request.marketDataTimestamp}` : null;
        if (snapshotKey) {
            const existing = await prisma_1.prisma.analysis.findFirst({ where: { userId, snapshotKey }, select: { id: true } });
            if (existing)
                return existing;
        }
        const result = await prisma_1.prisma.$transaction([
            prisma_1.prisma.user.upsert({ where: { id: userId }, update: {}, create: { id: userId } }),
            prisma_1.prisma.analysis.create({
                data: {
...
async function listHistory(options = {}, userId) {
    ...
    if (options.startDate || options.endDate) {
        ...
        if (options.startDate)
            analysisTimestamp.gte = new Date(options.startDate);
        if (options.endDate)
            analysisTimestamp.lte = new Date(options.endDate);
        where.analysisTimestamp = analysisTimestamp;
    }
    if (options.minConfidence !== undefined || options.maxConfidence !== undefined) {
        const confidenceScore = {};
        if (options.minConfidence !== undefined)
            confidenceScore.gte = options.minConfidence;
        if (options.maxConfidence !== undefined)
            confidenceScore.lte = options.maxConfidence;
        where.confidenceScore = confidenceScore;
    }
    const [analyses, total] = await Promise.all([
        prisma_1.prisma.analysis.findMany({
            where,
            select: {
                id: true,
                symbol: true,
        ...
        prisma_1.prisma.analysis.count({ where }),
    ]);
    return {
...
async function getAnalysisDetail(id, userId) {
    const analysis = await prisma_1.prisma.analysis.findFirst({
        where: { id, userId },
        include: {
            indicators: true,
            structureSnapshots: {
    ...
    return {
        ...
        historicalCandles: analysis.historicalCandles,
        ...
        trendFactors: analysis.trendFactors,
        ...
        tradeQualityReasons: analysis.tradeQualityReasons,
        ...
        setupConditionsMet: analysis.setupConditionsMet,
        setupConditionsMissing: analysis.setupConditionsMissing,
        ...
        aiStructured: analysis.aiStructured,
        ...
        structureSnapshot: analysis.structureSnapshots[0]
            ? {
                ...
                events: analysis.structureSnapshots[0].events,
...
async function deleteAnalysis(id, userId) {
    try {
        const owned = await prisma_1.prisma.analysis.findFirst({ where: { id, userId }, select: { id: true } });
        if (!owned)
            return false;
        await prisma_1.prisma.analysis.delete({ where: { id: owned.id } });
        return true;
    }
    catch (error) {
