...
exports.isCandleCorrupted = isCandleCorrupted;
...
function validateCandle(candle, index) {
    ...
    if (!hasInvalidPrice) {
        ...
        if (high < open)
            relationshipErrors.push('high < open');
        if (high < close)
            relationshipErrors.push('high < close');
        if (low > open)
            relationshipErrors.push('low > open');
        if (low > close)
            relationshipErrors.push('low > close');
        if (high < low)
            relationshipErrors.push('high < low');
...
function isCandleCorrupted(candle, index) {
    return validateCandle(candle, index).some((issue) => issue.severity === 'error');
}
//# sourceMappingURL=candleValidator.js.map