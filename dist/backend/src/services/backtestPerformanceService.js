...
exports.BacktestPerformanceTracker = void 0;
exports.calculateBacktestMetrics = calculateBacktestMetrics;
function unrealizedPnl(trade, price) {
...
class BacktestPerformanceTracker {
    ...
    recordCandle(candle, openTrades, newlyClosedTrades = []) {
        for (const trade of newlyClosedTrades) {
            if (this.appliedTradeIds.has(trade.id))
                throw new Error(`Trade ${trade.id} was applied to account balance more than once.`);
            if (trade.result === 'open' || trade.realizedPnl === null)
                throw new Error(`Trade ${trade.id} is not a completed trade with realized P/L.`);
    ...
    getDrawdownPeriods() { return [...this.drawdownPeriods]; }
    finishDrawdown(drawdown, recoveryTimestamp) {
...
exports.BacktestPerformanceTracker = BacktestPerformanceTracker;
...
function calculateBacktestMetrics(trades, equityCurve, initialBalance) {
    ...
    const realizedRs = completed.map((trade) => trade.realizedR).filter((value) => value !== null);
    const plannedRrs = trades.map((trade) => trade.plannedRiskReward).filter(Number.isFinite);
    const holdingMinutes = completed.map((trade) => trade.holdingMinutes).filter((value) => value !== null);
...
//# sourceMappingURL=backtestPerformanceService.js.map