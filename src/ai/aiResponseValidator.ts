...
function containsUnsafeGuarantee(text: string): boolean {
  ...
  while ((match = guarantee.exec(text)) !== null) {
    ...
    if (!/\b(?:not|never|no|without)\s+(?:a\s+)?$/i.test(prefix)) return true;
  }
  return false;
}

export function validateAiText(text: string): AiValidationResult {
...
export function validateAiFacts(text: string, context: AnalysisContext): AiValidationResult {
...
export const AI_UNAVAILABLE_MESSAGE = 'AI explanation unavailable for this analysis.';
...
export function validateStructuredAiResponse(text: string, context?: AnalysisContext): StructuredAiValidationResult {
  ...
  const invalid = [
    ...stringFields.filter((field) => typeof record[field] !== 'string' || !(record[field] as string).trim()),
    ...listFields.filter((field) => !Array.isArray(record[field]) || !(record[field] as unknown[]).every((item) => typeof item === 'string' && item.trim())),
  ];
  if (!Number.isInteger(record.confidence) || (record.confidence as number) < 0 || (record.confidence as number) > 100) {
    invalid.push('confidence must be an integer from 0 to 100');
  }
  if (invalid.length > 0) return { valid: false, matchedPatterns: invalid, output: null };

  const output = record as unknown as AiStructuredOutput;
  const safety = validateAiText(JSON.stringify(output));
  if (!safety.valid) return { ...safety, output: null };
