...
import { AnalysisContext, AiAnalysisResponse, AiStructuredOutput } from '../../../shared/types/aiAnalysis';
import { createLogger } from '../utils/logger';
import { AiProvider, AiProviderError } from './aiProvider';
import { validateAiFacts, validateStructuredAiResponse } from './aiResponseValidator';
...
function contextHash(context: AnalysisContext): string {
  ...
  const stable = JSON.parse(JSON.stringify(context, (key, value) => {
    if (key === 'analyzedAt' || key === 'generatedAt' || key === 'candleAgeMs') return undefined;
    return value;
  }));
  return crypto.createHash('sha256').update(JSON.stringify(stable)).digest('hex');
}

function primitiveRaw(raw: Record<string, unknown>): Record<string, unknown> {
 ...
 */
export function buildAiPromptContext(context: AnalysisContext): Record<string, unknown> {
  ...
  return {
    ...
    momentum: {
      ...
      components: Object.fromEntries(
        Object.entries(momentum.components ?? {}).map(([name, component]) => [name, {
...
function buildUserPrompt(context: AnalysisContext, stricter = false): string {
  ...
  if (serialized.length > MAX_PROMPT_CONTEXT_CHARS) {
    const reduced = {
      ...compactContext,
      momentum: {
        ...(compactContext.momentum as Record<string, unknown>),
        components: undefined,
      },
      evidenceAgreement: {
        ...(compactContext.evidenceAgreement as Record<string, unknown>),
        factors: undefined,
        compositeBreakdown: undefined,
      },
      supportResistance: (compactContext.supportResistance as unknown[]).slice(0, 6),
      setups: (compactContext.setups as unknown[]).slice(0, 3),
      multiTimeframeSnapshots: {
        ...(compactContext.multiTimeframeSnapshots as Record<string, unknown>),
...
export class AiAnalysisService {
  private readonly cache = new Map<string, AiAnalysisResponse>();
  private readonly usage: AiUsageStats = { requests: 0, providerAttempts: 0, cacheHits: 0, failures: 0, lastRequestAt: null };
  ...
  async explain(context: AnalysisContext): Promise<AiAnalysisResponse> {
    ...
    if (this.providers.length === 0) return this.unavailable(context);

    let first: { text: string; provider: string };
    try {
    ...
    let structuredValidation = validateStructuredAiResponse(explanation, context);
    let validation = structuredValidation;
    const factValidation = validateAiFacts(explanation, context);
    if (!factValidation.valid) validation = { ...validation, valid: false, matchedPatterns: [...validation.matchedPatterns, ...factValidation.matchedPatterns] };
    ...
    if (!validation.valid) {
      ...
      structuredValidation = validateStructuredAiResponse(explanation, context);
      validation = structuredValidation;
      const retryFactValidation = validateAiFacts(explanation, context);
      if (!retryFactValidation.valid) validation = { ...validation, valid: false, matchedPatterns: [...validation.matchedPatterns, ...retryFactValidation.matchedPatterns] };
    }
  ...
  private async generateWithFallback(prompt: string): Promise<{ text: string; provider: string }> {
    ...
    for (const provider of this.providers) {
      ...
      try {
        ...
        const kind = error instanceof AiProviderError ? error.kind : 'provider_error';
  ...
  private unavailable(context: AnalysisContext): AiAnalysisResponse {
...
function buildDeterministicFallback(context: AnalysisContext): AiStructuredOutput {
