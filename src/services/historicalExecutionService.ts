...
import { getEntryEligibility } from './historicalStrategyEvaluator';
...
function nearestBelow(price: number, values: number[]): number | null {
  return values.filter((value) => value < price).sort((a, b) => b - a)[0] ?? null;
}

function nearestAbove(price: number, values: number[]): number | null {
  return values.filter((value) => value > price).sort((a, b) => a - b)[0] ?? null;
}

function historicalStop(setup: BacktestSetupRecord, entryPrice: number, execution: BacktestExecutionAssumptions): number {
  const { direction, snapshot } = setup;
  if (execution.stopLossModel === 'atr') {
    const atr = lastValue(snapshot.indicators.atr14);
    if (atr === null || atr <= 0 || execution.atrStopMultiplier <= 0) throw new Error('ATR stop requires a positive historical ATR and multiplier.');
  ...
  if (level === null) throw new Error('Structure stop requires a confirmed historical level on the correct side of entry.');
  return level;
}

function historicalTarget(setup: BacktestSetupRecord, entryPrice: number, stopPrice: number, execution: BacktestExecutionAssumptions): number {
  const { direction, snapshot } = setup;
  const riskDistance = Math.abs(entryPrice - stopPrice);
  if (riskDistance <= 0) throw new Error('Stop must be a non-zero distance from entry.');

  if (execution.takeProfitModel === 'risk_reward') {
    if (execution.riskRewardRatio <= 0) throw new Error('Risk/reward ratio must be positive.');
  ...
  if (execution.takeProfitModel === 'atr') {
    const atr = lastValue(snapshot.indicators.atr14);
    if (atr === null || atr <= 0 || execution.atrTargetMultiplier <= 0) throw new Error('ATR target requires a positive historical ATR and multiplier.');
  ...
  if (execution.takeProfitModel === 'price') {
    if (execution.fixedTargetPrice === undefined || !Number.isFinite(execution.fixedTargetPrice)) throw new Error('Fixed-price target requires fixedTargetPrice.');
  ...
  if (target === null) throw new Error('Support/resistance target requires a historical level on the correct side of entry.');
  return target;
}

export function buildHistoricalExecutionPlan(
  setup: BacktestSetupRecord,
  candles: Candle[],
  execution: BacktestExecutionAssumptions
): BacktestExecutionPlan {
  const eligibility = getEntryEligibility(setup.snapshot.decisionIndex, candles, execution.entryModel, execution.entryPriceLevel);
  ...
  const pipValue = getInstrumentConfig(setup.snapshot.symbol).pipValue;
  ...
  if (setup.direction === 'bullish' && (stopPrice >= entryPrice || targetPrice <= entryPrice)) throw new Error('Bullish execution plan has invalid stop or target direction.');
  if (setup.direction === 'bearish' && (stopPrice <= entryPrice || targetPrice >= entryPrice)) throw new Error('Bearish execution plan has invalid stop or target direction.');
...
export function calculateHistoricalPositionSize(
  setup: BacktestSetupRecord,
  ...
  const instrument = getInstrumentConfig(setup.snapshot.symbol);
  if (fixedPositionUnits !== undefined) {
    if (!Number.isFinite(fixedPositionUnits) || fixedPositionUnits <= 0) throw new Error('Fixed position units must be positive.');
    return {
  ...
  if (!calculation) throw new Error('Position size cannot be calculated for this instrument and stop distance.');
  return {
