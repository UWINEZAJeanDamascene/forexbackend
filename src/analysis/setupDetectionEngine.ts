...
const logger = createLogger('setupDetection');
...
function higherTfTrend(ctx: SetupContext, expected: 'bullish' | 'bearish'): boolean {
  if (!higherTfOk(ctx)) return false;
  return ctx.multiTimeframe.higherTimeframe?.trend === expected;
}

export const SETUP_DEFINITIONS: SetupRule[] = [
  {
  ...
  },
  {
    ...
    invalidationCondition: (ctx) => {
      const parts: string[] = ['momentum flips bearish'];
  ...
  },
  {
    ...
    invalidationCondition: (ctx) => {
      const parts: string[] = ['momentum flips bullish'];
      if (ctx.volatility.classification !== 'high') {
        parts.push('volatility spikes to high');
      }
      return parts.join(' or ');
 ...
 */
function consensusDirection(ctx: SetupContext): 'bullish' | 'bearish' | 'neutral' {
...
function setupPriorityScore(setup: DetectedSetup, consensus: 'bullish' | 'bearish' | 'neutral'): number {
...
export function detectSetups(ctx: SetupContext): DetectedSetup[] {
  ...
  for (const rule of SETUP_DEFINITIONS) {
    ...
    for (const condition of rule.conditions) {
      ...
      if (passed) {
        ...
        conditionsMissing.push(
          condition.requiresHigherTf && mtfIncomplete
