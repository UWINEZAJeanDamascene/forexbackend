...
import { MomentumAnalysisResult, MomentumDirection, MomentumStrength, MomentumComponent } from '../../shared/types/momentumAnalysis';
...
function scoreRsi(rsiSeries: (number | null)[], trendContext: MomentumDirection): MomentumComponent {
...
function scoreMacd(
  ...
  const lineValues = macd.line.filter((value): value is number => value !== null);
  const signalValues = macd.signal.filter((value): value is number => value !== null);
  const histogramValues = macd.histogram.filter((value): value is number => value !== null);
...
function scorePriceMovement(candles: Candle[], atr: number | null): MomentumComponent {
  ...
  for (let i = recent.length - 1; i > 0; i--) {
    ...
    if (Math.abs(streak) >= MAX_STREAK_CAP) break;
  }
...
function classifyMomentum(score: number): MomentumDirection {
...
function normalizeTrend(trend: MarketStructureResult['trend']): MomentumDirection {
  switch (trend) {
    case 'bullish':
      return 'bullish';
    case 'bearish':
...
function detectDivergence(candles: Candle[], rsiSeries: (number | null)[]): 'bullish' | 'bearish' | null {
  const swingWindow = 2;
...
function findSwingHighs(candles: Candle[], window: number): { price: number; index: number }[] {
...
function findSwingLows(candles: Candle[], window: number): { price: number; index: number }[] {
  const swings: { price: number; index: number }[] = [];
  const len = candles.length;
...
function getRsiAtCandle(rsiSeries: (number | null)[], candleIndex: number): number | null {
  if (candleIndex < 0 || candleIndex >= rsiSeries.length) return null;
