...
import {
  ...
  TrendDirection,
  TrendFactor,
} from '../../shared/types/trendAnalysis';
...
function normalizeByAtr(delta: number, atr: number | null): number {
  if (atr === null || atr <= 0) return delta > 0 ? 1 : delta < 0 ? -1 : 0;
  return delta / atr;
}
...
function sideVsEma(price: number, ema: number | null): TrendDirection | null {
...
function buildPriceVsEmaBreakdown(
  price: number | null,
  ema20: number | null,
  ema50: number | null,
  ema200: number | null
...
function analyzePriceVsEma(
  ...
  if (primary === null) {
    const sides = [breakdown.vsEma50, breakdown.vsEma200].filter((s): s is TrendDirection => s !== null);
  ...
  const longer = [breakdown.vsEma50, breakdown.vsEma200].filter((s): s is TrendDirection => s !== null && s !== 'neutral');
  if (longer.length > 0) {
...
function deriveStrength(score: number): 'weak' | 'moderate' | 'strong' {
