...
import { getConfidenceAnalysis } from '../analysis/confidenceAnalysisService';
import { createLogger } from '../utils/logger';

const logger = createLogger('confidence.controller');
...
export async function getConfidenceEndpoint(req: Request, res: Response): Promise<void> {
  ...
  try {
    const result = await getConfidenceAnalysis(symbol, timeframe);
